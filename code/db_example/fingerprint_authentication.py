...
def main():
    ...
    cursor_selected_user = create_db_cursor()
    
    # Set a similarity threshold
    threshold = 4
    
    # Set failed attempts limit
    failed_attempts_limit = 5
    
    print(f"## Welcome to fingerprint authentication app! ##")
    ...
    while authentifing:
        ...
        execute_command_and_log(cursor_fingerprint, "SELECT id, fingerprint_data, user_id FROM fingerprint WHERE user_id = ? ORDER BY id", (str(selected_user_id)))
        ...
        while fingerprint_db:
            try:
                ...
                if match_score < threshold:
                    ...
                    execute_command_and_log(cursor, "UPDATE users SET failed_attempts = ?, bloqued_until = ? WHERE id = ?", (str(0), None, str(selected_user_id)))
        ...
        if authentifing == True:
            ...
            if failed_attempts >= failed_attempts_limit:
                ...
                # Add x minutes to the bloqued time
                new_bloqued_until = now + timedelta(minutes=(failed_attempts / failed_attempts_limit))

                execute_command_and_log(cursor, "UPDATE users SET bloqued_until = ? WHERE id = ?", (str(new_bloqued_until), str(selected_user_id)))
